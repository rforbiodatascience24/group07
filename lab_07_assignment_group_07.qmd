...
We only wish to look at a few of the statistics for our PCA, so we create a new tibble with only the abive mentioned attributes. We also replace `NA` values with 0, as these are caused by division by 0 when the player has had 0 attempts.
...
Our data is plotted over the two first principal components and color coded according to the player position.
